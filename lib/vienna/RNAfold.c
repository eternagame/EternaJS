...
AS3_Val rnaFold(int argc, char *argv[], char *data)
{
  char *string, *line;
  ...
  do {				/* main loop: continue until end of file */
    ...
    //}
   // fname[0]='\0';
    if ((line = get_line(data))==NULL) break;
    ...
    if (fold_constrained) {
		//TODO: instead of stdin make it the last argument?
      cstruc = get_line(data);
      if (cstruc!=NULL)
	...
	//TODO: Figure out how big ffname needs to be
	...
	if (pf) {
	    ...
	    if (do_backtrack) {
			...
			if (do_backtrack==2) {
			  ...
			  PS_dot_plot_list(string, ffname, pl1, pl2, "Probabilities for stacked pairs (i,j)(i+1,j-1)");
			  //free(pl2);
			}
			//free(pl1);
			//free(pf_struc);
			if (doMEA) {
			  ...
			  //TODO: need to ret mea and mea_en
			  free(pl);
			}
	    }
		
	    //printf(" frequency of mfe structure in ensemble %g; ",
		//this had an extra parens.. hmm
  ...
  //return 0;
  //returns an as3 array of everything, even if some are 0/null
  return AS3_Array("StrType", "StrType", "DoubleType", "StrType", "DoubleType", "DoubleType", "DoubleType", string, structure, min_en, pf_struc, energy, mea, mea_en);
}
