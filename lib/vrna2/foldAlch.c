...
// Sends debug output to javascript console through Flash
void TraceJS(char* text)
{
  AS3_Val extNS = AS3_String("flash.external");
  AS3_Val ext_cls = AS3_NSGetS(extNS, "ExternalInterface");
  AS3_Release(AS3_CallTS("call", ext_cls, "StrType, StrType, StrType", "console.log", "(Alch)", text));
  AS3_Release(ext_cls);
  AS3_Release(extNS);
}
...
static AS3_Val loadParams(void* self, AS3_Val args)
{
	char* filename = NULL;
	AS3_Val param_args;
	int ret;
	...
	ret = read_parameter_file(filename);
	
	temperature = 37.;
	update_fold_params(); // needed?

	return AS3_Array("IntType", ret);
}
...
int main()
{
	...
	AS3_Val loadParamsMethod = AS3_Function(NULL, loadParams );
	// construct an object that holds references to the functions
	AS3_Val result = AS3_Object("fullAlchFold: AS3ValType, AlchEval: AS3ValType, fullAlchEval: AS3ValType, loadParams: AS3ValType", 
					fullFoldMethod, evalMethod, fullEvalMethod, loadParamsMethod);

	// Release
	AS3_Release( fullFoldMethod );
	AS3_Release( evalMethod );
	AS3_Release( fullEvalMethod );
	AS3_Release( loadParamsMethod );
