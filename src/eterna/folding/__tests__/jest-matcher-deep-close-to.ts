// Taken from https://github.com/maasencioh/jest-matcher-deep-close-to and adapted for typescript

type MatcherUtils = jest.MatcherUtils;
 ...
 */
export function toBeDeepCloseTo(this: MatcherUtils, received: any, expected: any, decimals: number) {
    if (decimals === undefined) {
        decimals = 10;
    }
    var error: any = recursiveCheck(received, expected, decimals);
 ...
 * @return {{message: (function(): string), pass: boolean}}
 */
export function toMatchCloseTo(this: MatcherUtils, received: any, expected: any, decimals: number) {
    var error: any = recursiveCheck(received, expected, decimals, false);
    /* istanbul ignore next */
    if (error) {
        return {
...
function recursiveCheck(actual: any, expected: any, decimals: number, strict: boolean = true) {
    if (typeof actual === 'number' && typeof expected === 'number') {
        ...
        for (var i = 0; i < actual.length; i++) {
            var error: any = recursiveCheck(actual[i], expected[i], decimals, strict);
        ...
        for (const prop in expected) {
            var properror: any = recursiveCheck(actual[prop], expected[prop], decimals, strict);
