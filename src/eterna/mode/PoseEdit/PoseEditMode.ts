...
import {downloadRNAProPDB, getRNAProResult, requestMorph} from 'eterna/folding/RNAProFolder';
...
export default class PoseEditMode extends GameMode {
    ...
    public openRNAPro3D(): void {
        ...
        if (!dialog) return;
        this._pose3D = dialog;
        this._pose3DShownSeq = latest.sequence;
        this._pose3DShownPdb = latest.pdb;
        ...
        dialog.setRNAProStatus(this.rnaProStatusLine(latest.sequence, latest.secstruct, latest.walltimeS));

        // Subsequent structure updates (edits, cached structures, undo/redo, mode toggle) are driven
        // by refresh3DForCurrentPose() from updateScore() — see that method.
        dialog.closed.then(() => {
            if (this._pose3D === dialog) {
                this._pose3D = null;
                this._pose3DShownSeq = undefined;
                this._pose3DShownPdb = undefined;
            }
        });
    }

    private rnaProStatusLine(seq?: string, ss?: string, wt?: number | null): string {
        return `RNAPro · ${seq} · ${ss}${wt != null ? ` · ${wt}s` : ''}`;
    }

    /**
     * Keep the open 3D view in sync with the currently-displayed pose. Runs from updateScore(), so it
     * fires for every 2D-updating action — edits, cached structures, undo/redo, natural/target toggle.
     * Morphs (via the bridge /morph) from the structure currently shown to the one for this sequence.
     */
    private refresh3DForCurrentPose(): void {
        const dialog = this._pose3D;
        if (!dialog || !this.isNaturalMode) return;
        const ublk = this.getCurrentUndoBlock(0);
        const seqStr = ublk ? this.transformSequence(ublk.sequence, 0, 0).sequenceString() : undefined;
        if (!seqStr || seqStr === this._pose3DShownSeq) return;
        const result = getRNAProResult(seqStr);
        if (!result || !result.pdb) return; // not folded yet; the fold + next updateScore will handle it

        const fromPdb = this._pose3DShownPdb;
        this._pose3DShownSeq = seqStr; // claim synchronously so we don't double-fire
        const seq = Sequence.fromSequenceString(seqStr);
        const ss = SecStruct.fromParens(result.secstruct || '.'.repeat(seq.length), true);
        dialog.setRNAProStatus(this.rnaProStatusLine(seqStr, result.secstruct, result.walltimeS));

        if (!fromPdb) {
            this._pose3DShownPdb = result.pdb;
            dialog.updateStructure(result.pdb, seq, ss);
            return;
        }
        requestMorph(fromPdb, result.pdb).then(({pdb, morphFrom, morphTo}) => {
            if (this._pose3D !== dialog || this._pose3DShownSeq !== seqStr) return; // superseded
            this._pose3DShownPdb = pdb;
            dialog.updateStructure(pdb, seq, ss, morphFrom, morphTo);
        }).catch((e) => {
            // eslint-disable-next-line no-console
            console.error('[RNAPro] morph failed:', e);
            if (this._pose3D === dialog && this._pose3DShownSeq === seqStr) {
                this._pose3DShownPdb = result.pdb;
                dialog.updateStructure(result.pdb, seq, ss); // fall back to a plain reload
            }
        });
    }
    ...
    private setPuzzle(): void {
        ...
        for (let ii = 0; ii < this._poses.length; ii++) {
            ...
            this._poses[ii].sequence = this.transformSequence(seq, this.poseTargetIndex(ii), 0);
    ...
    private buildScriptInterface(forConstraints: boolean = false): ExternalInterfaceCtx {
        ...
        scriptInterfaceCtx.addCallback(
            'get_sequence_string',
            lockDuringFold((): string => this.transformSequence(
        ...
        scriptInterfaceCtx.addCallback(
            ...
            lockDuringFold(
                async (seq: string): Promise<boolean> => {
                    ...
                    await this.pasteSequence(sequence, 0);
                    return true;
                }
            )
        );
    ...
    protected createContextMenu(poseIdx: number): ContextMenu | null {
        ...
        menu.addItem('Copy Sequence').clicked.connect(() => this.showCopySequenceDialog(poseIdx));
        menu.addItem('Paste Sequence').clicked.connect(() => this.showPasteSequenceDialog(poseIdx));
    ...
    private changeTarget(targetIndex: number): void {
        ...
        if (this._poseState === PoseState.NATIVE) {
            this._poses[0].sequence = this.transformSequence(
    ...
    private setPoseTarget(poseIndex: number, targetIndex: number): void {
        if (this._targetConditions[targetIndex] !== undefined) {
            const tc = this._targetConditions[targetIndex] as TargetConditions;
            this._poses[poseIndex].sequence = this.transformSequence(
                this.getCurrentUndoBlock(targetIndex).sequence, targetIndex, targetIndex
            );
            const locks = this.getCurrentUndoBlock(targetIndex).puzzleLocks;
            if (locks) {
    ...
    private createSubmitData(details: SubmitPoseDetails, undoBlock: UndoBlock): SubmitSolutionData {
        ...
        const seqString: string = this.transformSequence(undoBlock.sequence, 0, 0).sequenceString();
    ...
    private async submitSolution(
        ...
        const seq = this.transformSequence(undoBlock.sequence, 0, 0);
    ...
    public transformSequence(seq: Sequence, targetIndex: number, fromTargetIndex: number): Sequence {
        ...
        const targetSeq = this._targetConditions[targetIndex]?.['sequence'];
        if (targetSeq) {
            const ublk = this.getCurrentUndoBlock(targetIndex);
            const locks = ublk.puzzleLocks;

            if (locks && (locks.length !== newSequence.length || newSequence.length !== seq.length)) {
            ...
            for (let ii = 0; ii < targetSeq.length; ii++) {
                if (locks?.[ii]) {
                    seq.setNt(ii, EPars.stringToNucleotide(targetSeq[ii]));
                }
            }
        }
    ...
    private updateScore(): void {
        ...
        if (this._pose3D) {
            this._pose3D.sequence.value = this.transformSequence(this.getCurrentUndoBlock(0).sequence, 0, 0);
            // Morph the 3D structure to match the now-displayed pose (edits, cached, undo/redo, toggle).
            this.refresh3DForCurrentPose();
        }

        if (this._shouldMarkMutations) {
            const puzzleInitSeq = this._puzzle.getBeginningSequence();
            const currentSeq = this.transformSequence(this.getCurrentUndoBlock(0).sequence, 0, 0);
    ...
    private syncUpdatesFromPose(sourcePoseIndex: number): void {
        ...
        for (let ii = 0; ii < this._poses.length; ii++) {
            ...
            this._poses[ii].sequence = this.transformSequence(
    ...
    private async poseEditByTargetFoldTarget(ii: number, sync: boolean): Promise<void> {
        ...
        const poseTargetIdx = this.poseTargetIndex(ii);
        const seq: Sequence = this.transformSequence(this._poses[ii].sequence, ii, poseTargetIdx);
    ...
    private moveUndoStackForward(): void {
        ...
        const before: Sequence = this.transformSequence(this.getCurrentUndoBlock(0).sequence, 0, 0);

        this._stackLevel++;
        this.moveUndoStack();

        const after: Sequence = this.transformSequence(this.getCurrentUndoBlock(0).sequence, 0, 0);
    ...
    private moveUndoStackToLastStable(): void {
        ...
        while (this._stackLevel >= 1) {
            if (this.getCurrentUndoBlock(0).stable) {
                ...
                const after: Sequence = this.transformSequence(
                    this.getCurrentUndoBlock(0).sequence, 0, 0
                );
    ...
    // RNAPro demo: the sequence + PDB currently shown in the 3D view, so refresh3DForCurrentPose()
    // only morphs on an actual change and knows what to morph "from".
    private _pose3DShownSeq?: string;
    private _pose3DShownPdb?: string;
