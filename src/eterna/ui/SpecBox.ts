...
export class SpecBox extends GamePanel {
    constructor(docked: boolean = false) {
        ...
        this._zoom_in.display.position = new Point(40, this.get_panel_height() - 125);
        ...
        this._zoom_out.display.position = new Point(70, this.get_panel_height() - 130);
    ...
    public set_spec(datablock: UndoBlock): void {
        ...
        this._dotplot = datablock.get_dotplot();
        this._meltplot = datablock.get_meltplot();
    ...
    public getPlotSize(): number {
        ...
        if (this._docked) {
            plot_w = this.get_panel_width() - 55;
            plot_h = (this.get_panel_height() - 51) / 2.0;
        } else {
            plot_w = (this.get_panel_width() - 100) / 2.0;
            plot_h = this.get_panel_height() - 200;
        }
    ...
    private updateSpecBoxView(): void {
        let panelWidth = this.get_panel_width();
        let panelHeight = this.get_panel_height();

        if (this._docked) {
            this._dotplot_canvas.position = new Point(20, 15);
            this._meltplot_canvas.position = new Point(20, (panelHeight * 0.5) + 8);
        ...
        if (this._meltplot != null && plot_size > 0) {
            ...
            if (this._docked) {
                this._h0_melt.position = new Point(15, (panelHeight * 0.5) + plot_size + 10);
                this._hn_melt.position = new Point(
                    25 + plot_size - this._hn_melt.width,
                    (panelHeight * 0.5) + plot_size + 10);
                ...
                this._h0_melt.position = new Point((panelWidth * 0.5) + 15, plot_size + 75);
                this._hn_melt.position = new Point(
                    (panelWidth * 0.5) + 25 + plot_size - this._hn_melt.width,
                    plot_size + 75);
                this._v0_melt.position = new Point(
                    (panelWidth * 0.5) + 25 - this._v0_melt.width - 3,
                    55 + plot_size);
            }
        }
    }
    ...
    private _dotplot_canvas: Sprite;
    private _meltplot: Plot;
    private _meltplot_canvas: Sprite;
    private _h0: Text;
    private _h0_melt: Text;
    private _hn_melt: Text;
    private _v0: Text;
    private _v0_melt: Text;
    private _vn_melt: Text;
    ...
    private _zoom_in: GameButton;
    private _zoom_out: GameButton;
